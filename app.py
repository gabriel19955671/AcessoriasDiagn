...
# --- ROBUSTA: lê .xlsx/.xlsm com openpyxl, .xlsb com pyxlsb, .xls antigo com xlrd, e cai pra CSV se nada servir
def try_read_excel(uploaded_file) -> pd.DataFrame:
    """
    Lê planilhas Excel com detecção de formato e fallbacks:
    - .xlsx / .xlsm -> engine=openpyxl
    - .xlsb         -> engine=pyxlsb
    - .xls (antigo) -> engine=xlrd (fallback tenta openpyxl)
    - Se nada der certo, tenta CSV.
    """
    ...
    # Tentativa heurística se extensão não ajudou
    ...
    try:
        ...
        st.error(f"Não consegui abrir o arquivo como Excel nem como CSV: {e}")
        raise
...
def filter_panel(df, defaults=None, who="ent"):
    ...
    with st.expander("🎛️ Filtros", expanded=True):
        ...
        if date_candidates:
            ...
            if dcol == "<sem filtro de data>":
                dcol = None
        di = dfim = None
        if dcol:
            from_d, to_d = st.columns(2)
            with from_d:
                di = st.date_input("Data inicial", value=None, key=f"di_{who}")
            with to_d:
                dfim = st.date_input("Data final", value=None, key=f"df_{who}")

    mask = pd.Series(True, index=df.index)
    if emp:
        mask &= df["empresa"].isin(emp_sel)
    if dep:
        mask &= df["departamento"].isin(dep_sel)
    if res_cols and res_sel:
        mask &= df[res_cols].apply(lambda r: any([(x in res_sel) for x in r.values if pd.notna(x)]), axis=1)
    if status_vals:
        mask &= df["status"].isin(stt_sel)
    if dcol:
        col_dt = pd.to_datetime(df[dcol], errors="coerce")
        if di:
            mask &= col_dt.dt.date >= di
        if dfim:
            mask &= col_dt.dt.date <= dfim
...
with st.sidebar:
    ...
    up_solic    = st.file_uploader("Solicitações (XLSX/XLSM/XLSB/XLS/CSV)", type=["xlsx","xlsm","xlsb","xls","csv"])
    up_obrig    = st.file_uploader("Obrigações (XLSX/XLSM/XLSB/XLS/CSV)", type=["xlsx","xlsm","xlsb","xls","csv"])
    up_proc     = st.file_uploader("Gestão de Processos (XLSX/XLSM/XLSB/XLS/CSV)", type=["xlsx","xlsm","xlsb","xls","csv"])
    up_resp     = st.file_uploader("Responsáveis & Departamentos (XLSX/XLSM/XLSB/XLS/CSV)", type=["xlsx","xlsm","xlsb","xls","csv"])
...
with tabs[2]:
    if up_solic:
        ...
        if name.endswith((".xlsx",".xlsm",".xlsb",".xls")):
            raw = try_read_excel(up_solic)
        else:
            raw = read_any_csv(up_solic)
        raw = normalize_headers(raw)
        ...
        st.dataframe(dfs)
        st.caption(f"Linhas exibidas: {len(dfs):,}".replace(",","."))
        st.download_button("⬇️ Baixar CSV filtrado (Solicitações)", dfs.to_csv(index=False).encode("utf-8"), "solicitacoes_filtrado.csv", "text/csv")
    else:
...
with tabs[3]:
    if up_obrig:
        ...
        if name.endswith((".xlsx",".xlsm",".xlsb",".xls")):
            raw = try_read_excel(up_obrig)
        else:
            raw = read_any_csv(up_obrig)
        raw = normalize_headers(raw)
        ...
        st.dataframe(dfo)
        st.caption(f"Linhas exibidas: {len(dfo):,}".replace(",","."))
        st.download_button("⬇️ Baixar CSV filtrado (Obrigações)", dfo.to_csv(index=False).encode("utf-8"), "obrigacoes_filtrado.csv", "text/csv")
    else:
...
with tabs[4]:
    if up_proc:
        ...
        if name.endswith((".xlsx",".xlsm",".xlsb",".xls")):
            raw = try_read_excel(up_proc)
        else:
            raw = read_any_csv(up_proc)
        raw = normalize_headers(raw)
        ...
        st.dataframe(dfp)
        st.caption(f"Linhas exibidas: {len(dfp):,}".replace(",","."))
        st.download_button("⬇️ Baixar CSV filtrado (Processos)", dfp.to_csv(index=False).encode("utf-8"), "processos_filtrado.csv", "text/csv")
    else:
...
with tabs[5]:
    ...
    if isinstance(dfe, pd.DataFrame) and not dfe.empty:
        ...
        if "empresa" in dfc.columns and emp_sel:
            mask &= dfc["empresa"].isin(emp_sel)
        if "departamento" in dfc.columns and dep_sel:
            mask &= dfc["departamento"].isin(dep_sel)
        dfc = dfc[mask].copy()
...
with tabs[6]:
    ...
    if gerar:
        ...
        if isinstance(st.session_state.get("dfs"), pd.DataFrame):
            ...
            total_s = len(dfs)
            abertas = int(s_get(dfs,"conclusao").isna().sum() if "conclusao" in dfs.columns else total_s)
            linhas += [
                f"### Solicitações",
...
with tabs[7]:
    if up_resp:
        ...
        if name.endswith((".xlsx",".xlsm",".xlsb",".xls")):
            raw = try_read_excel(up_resp)
        else:
            raw = read_any_csv(up_resp)
        raw = normalize_headers(raw)
        ...
        st.dataframe(dfr)
        st.caption(f"Linhas exibidas: {len(dfr):,}".replace(",","."))
        st.download_button("⬇️ Baixar CSV filtrado (Responsáveis)", dfr.to_csv(index=False).encode("utf-8"), "responsaveis_filtrado.csv", "text/csv")
    else:
...
with tabs[0]:
    ...
    if isinstance(st.session_state.get("dfs"), pd.DataFrame):
        ...
        total = len(dfs)
        abertas = int(s_get(dfs,"conclusao").isna().sum() if "conclusao" in dfs.columns else total)
