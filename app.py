# app.py — Dashboard Acessórias (arquivo único)
...
st.caption("Upload por cliente → mapeie colunas → aplique filtros → calcule métricas (prazo técnico/legal) → gere relatórios.")

# ============== Helpers de leitura (robustos) ==============
def read_any_csv(uploaded_file) -> pd.DataFrame:
    """
    Lê CSV tentando múltiplas codificações e separadores.
    Encodings: utf-8, utf-8-sig, latin1, cp1252, iso-8859-1, utf-16(le/be).
    Separadores: auto, ; , , | e TAB.
    """
...
def try_read_excel(uploaded_file) -> pd.DataFrame:
    ...
    - .xlsx/.xlsm -> openpyxl
    - .xlsb       -> pyxlsb
    - .xls        -> xlrd (fallback openpyxl)
    - Se tudo falhar: tenta CSV (read_any_csv)
    """
    ...
    if name.endswith(".xls"):
        ...
        if df is not None: return df
        # muita gente salva .xlsx com extensão .xls
        df = _try(pd.read_excel, uploaded_file, engine="openpyxl", dtype=str)
        if df is not None: return df
...
def mapping_wizard(df, title, required, optional, key):
    ...
    st.caption("Use **Auto-detectar** e ajuste manualmente se precisar. Campos obrigatórios e opcionais abaixo.")
    ...
    merged = mapped_req.copy()
    merged.update({k:v for k,v in mapped_opt.items() if v})
    return merged
...
def filter_panel(df, defaults=None, who="ent"):
    ...
    return df[mask].copy(), dcol, di, dfim
...
with st.sidebar:
    ...
    up_solic    = st.file_uploader("Solicitações (XLS/XLSX/XLSM/XLSB/CSV)", type=["xls","xlsx","xlsm","xlsb","csv"])
    up_obrig    = st.file_uploader("Lista de Obrigações (XLS/XLSX/XLSM/XLSB/CSV)", type=["xls","xlsx","xlsm","xlsb","csv"])
    up_proc     = st.file_uploader("Gestão de Processos (XLS/XLSX/XLSM/XLSB/CSV)", type=["xls","xlsx","xlsm","xlsb","csv"])
    up_resp     = st.file_uploader("Responsáveis & Departamentos (XLS/XLSX/XLSM/XLSB/CSV)", type=["xls","xlsx","xlsm","xlsb","csv"])
...
with tabs[1]:
    if up_entregas:
        ...
        ent = raw.rename(columns=mapping)
        ...
        dfe, dcol, di, dfim = filter_panel(ent, who="ent")
        ...
        k1.metric("Total", f"{total:,}".replace(",",".")); k2.metric("Concluídas", f"{concl:,}".replace(",",".")); k3.metric("Pendentes", f"{pend:,}".replace(",",".")); k4.metric("Atrasadas (legal)", f"{atrasos:,}".replace(",",".")); 
        ...
        st.markdown("#### ⚙️ Atraso Técnico — Filtros e Rankings")
        ...
        st.caption(f"Linhas exibidas: {len(dfe):,}".replace(",",".")); st.download_button("⬇️ Baixar CSV filtrado (Entregas)", dfe.to_csv(index=False).encode("utf-8"), "entregas_filtrado.csv", "text/csv")
    else:
...
with tabs[2]:
    if up_solic:
        ...
        sol = raw.rename(columns=mapping)
        ...
        today = pd.to_datetime(date.today())
        sol["tempo_ate_conclusao_dias"] = np.where(s_get(sol,"conclusao").notna() & s_get(sol,"abertura").notna(), s_dt_days(s_get(sol,"conclusao") - s_get(sol,"abertura")), np.nan)
        sol["aberta_ha_dias"] = np.where(s_get(sol,"conclusao").isna() & s_get(sol,"abertura").notna(), s_dt_days(pd.to_datetime(today) - s_get(sol,"abertura")), np.nan)

        dfs, _, _, _ = filter_panel(sol, who="sol")
        st.session_state["dfs"] = dfs

        total = len(dfs); concl = int(s_get(dfs,"status").astype(str).str.lower().eq("concluída").sum()); abertas = total - concl
        k1,k2,k3 = st.columns(3)
        k1.metric("Total", f"{total:,}".replace(",",".")); k2.metric("Abertas", f"{abertas:,}".replace(",",".")); k3.metric("SLA médio (dias)", f"{float(np.nanmean(s_get(dfs,'tempo_ate_conclusao_dias'))):,.1f}".replace(",","."))
...
with tabs[3]:
    if up_obrig:
        ...
        # Mapeamento típico de uma "Lista de Obrigações"
        required = ["obrigacao","departamento"]
        optional = ["mini","empresa","responsavel","periodicidade","prazo_mensal","alerta_dias","observacao"]

        mapping = mapping_wizard(raw, "Lista de Obrigações", required, optional, "obr")
        obr = raw.rename(columns=mapping)

        # Converte possíveis datas (se existirem colunas de datas no arquivo)
        date_like = [c for c in ["prazo_mensal"] if c in obr.columns]
        obr = to_datetime_cols(obr, date_like)

        dfo, _, _, _ = filter_panel(obr, who="obr")
        st.session_state["dfo"] = dfo

        # Visualização: árvore Depto → Obrigação (se colunas existirem)
        if {"departamento","obrigacao"}.issubset(dfo.columns) and not dfo.empty:
            try:
                st.plotly_chart(px.treemap(dfo, path=["departamento","obrigacao"], title="Portfólio de Obrigações por Departamento"), use_container_width=True)
            except Exception:
                st.dataframe(dfo.groupby(["departamento","obrigacao"]).size().reset_index(name="qtd"))

        st.markdown("#### Lista de Obrigações (com filtros aplicados)")
        st.dataframe(dfo)
        st.caption(f"Linhas exibidas: {len(dfo):,}".replace(",",".")); st.download_button("⬇️ Baixar CSV filtrado (Obrigações)", dfo.to_csv(index=False).encode("utf-8"), "obrigacoes_filtrado.csv", "text/csv")
    else:
        st.info("Envie **Lista de Obrigações**.")

# ============== PROCESSOS ==============
with tabs[4]:
    if up_proc:
        name = up_proc.name.lower()
        raw = try_read_excel(up_proc) if name.endswith(('.xls', '.xlsx', '.xlsm', '.xlsb')) else read_any_csv(up_proc)
        raw = normalize_headers(raw)
        ...
        pro = raw.rename(columns=mapping)
        pro = to_datetime_cols(pro, ["inicio","conclusao"])
        if "status" in pro.columns:
            pro["status"] = pro["status"].map(norm_status).fillna(pro["status"])

        dfp, _, _, _ = filter_panel(pro, who="pro")
        st.session_state["dfp"] = dfp

        total = len(dfp); concl = int(s_get(dfp,"status").astype(str).str.lower().eq("concluída").sum())
        k1,k2,k3 = st.columns(3)
        k1.metric("Processos", f"{total:,}".replace(",",".")); k2.metric("Concluídos", f"{concl:,}".replace(",",".")); 
...
with tabs[5]:
    ...
    st.caption("Atraso em relação ao **prazo técnico** e à **data legal** (conclusão > prazo = atraso).")
    ...
    if isinstance(dfe, pd.DataFrame) and not dfe.empty:
        ...
        for base_col in ["competencia","data_vencimento","data_entrega"]:
            if base_col in dfc.columns:
                try:
                    mes_series = pd.to_datetime(dfc[base_col], errors="coerce").dt.to_period("M").astype(str)
                    break
                except Exception:
                    pass
        ...
        st.markdown("---")
        st.markdown("### 🔥 Heatmaps de Atrasos (%%) — Empresa × Mês")
        if "empresa" in dfc.columns and "mes" in dfc.columns:
            if has_tecnico:
                ...
                if not base_t.empty:
                    ...
                    try:
                        st.plotly_chart(px.imshow(pt, aspect="auto", title="% Atraso Técnico — Empresa x Mês", labels=dict(color="%")), use_container_width=True)
                    except Exception:
                        st.dataframe(pt.round(1))
            else:
                st.info("Mapeie **prazo_tecnico** + **data_entrega** para heatmap técnico.")
            if has_legal:
                base_l = dfc[dfc["late_legal"].notna()]
                if not base_l.empty:
                    pl = base_l.pivot_table(index="empresa", columns="mes", values="late_legal", aggfunc="mean") * 100
                    try:
                        st.plotly_chart(px.imshow(pl, aspect="auto", title="% Atraso Legal — Empresa x Mês", labels=dict(color="%")), use_container_width=True)
                    except Exception:
                        st.dataframe(pl.round(1))
            else:
                st.info("Mapeie **data_legal** + **data_entrega** para heatmap legal.")
        else:
            st.info("Para heatmaps, é necessário ter **empresa** e uma coluna de data/competência.")

        st.markdown("---")
        st.markdown("### 📑 Exports (comparativo)")
    ...
    if gerar:
        ...
        if isinstance(st.session_state.get("dfe"), pd.DataFrame):
            ...
            if "data_vencimento" in dfe.columns:
                ...
                total = len(dfe); concluidas = int((s_get(dfe,"status").astype(str).str.lower()=="concluída").sum()); pendentes = total - concluidas
                atrasadas = int((dfe["atrasada_concluida"] | dfe["atrasada_pendente"]).sum()); em_risco_qtd = int(dfe["em_risco"].sum())
...
with tabs[7]:
    if up_resp:
        ...
        resp = raw.rename(columns=mapping)

        dfr, _, _, _ = filter_panel(resp, who="resp")
...
with tabs[0]:
    ...
    if isinstance(st.session_state.get("dfe"), pd.DataFrame):
        ...
        c1.metric("Entregas (total | atrasadas legais)", "—")
    if isinstance(st.session_state.get("dfs"), pd.DataFrame):
        ...
        c2.metric("Solicitações (total | abertas)", "—")
    if isinstance(st.session_state.get("dfp"), pd.DataFrame):
        dfp = st.session_state["dfp"]; total = len(dfp)
        c3.metric("Processos (total)", f"{total:,}".replace(",",".")) 
    else:
        c3.metric("Processos (total)", "—")
